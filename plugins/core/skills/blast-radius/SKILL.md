...
metadata:
  source: "ported from pstack blast-radius (Lauren Tan, github.com/cursor/plugins, MIT) on 2026-08-20; companion-skill references adapted"
---
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the PR description and commit history for intent.
...
3. Look where grep stops. Read the source of the library you call, and check its pinned version and any local patch. Work out when things run: microtasks, unmount and teardown, framework scheduling differences. Follow what a symbol search misses: the JSON an API returns, a DB column, a wire format, another language reading the same bytes, a feature flag, code three hops downstream.
...
6. For a big or wide change, fan out. Ask several independent subagents (or models) the same question and merge the answers. Different reviewers catch different real bugs.
...
Write it through `redpen`, cite real code, and strip anything private before it goes anywhere public.
