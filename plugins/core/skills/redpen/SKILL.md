...
Pipeline: **pick the register → apply the rules → run the quick checks → lint mechanically → score → hand off to power-law if political**.
...
## Step 3 — STE rules (strict and STE-flavored registers)
...
Rate 1–10 on each dimension; below 42/60, revise:
...
## Step 8 — power-law hand-off

If the text is a stakeholder communication, workplace message, negotiation, escalation, or anything with political stakes: after the style pass, **invoke the `power-law` skill (power-check mode)** on the revised draft. Redpen fixes how it reads; power-law checks whether it's the right move, to the right person, at the right time. Deliver both results together — style rewrite first, power check after.
...
Merged from: stop-slop by Hardik Pandya (hvpandya.com, MIT), humanizer (github.com/blader/humanizer, based on Wikipedia:Signs of AI writing), and an ASD-STE100 distillation.
