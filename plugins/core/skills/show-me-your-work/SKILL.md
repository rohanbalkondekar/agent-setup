...
metadata:
  source: "ported from pstack show-me-your-work (Lauren Tan, MIT) on 2026-08-19; transcript-audit section adapted for Claude Code"
---
...
- **evidence.** A link or path that proves it: commit SHA, MR/PR number, `file:line`, or an artifact, trace, or screenshot path. Never a paragraph.
- **result.** The outcome or predicate state: `tests green`, `reverted`, `pixel-diff 0`, `INCONCLUSIVE`, `open`.

An example, plain-spoken so a reviewer reads it at a glance. Illustration only; don't copy these rows into a real log.
...
Write each entry the way you'd tell a teammate what you did. Plain words, concrete actions, no AI speak or abstract jargon (the **redpen** skill applies to log text too). A reviewer should understand each row without decoding it.
...
At the end of the run, before handing back, check the log told the truth. Walk the log against what actually happened in this session (your own tool calls, diffs, and outputs; do not read other sessions' transcripts):
